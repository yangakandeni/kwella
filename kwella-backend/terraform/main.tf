...
# ---------------------------------------------------------------------------
# kwella Driver Document Storage
# ---------------------------------------------------------------------------
# Private bucket for driver onboarding documents (license, PrDP, etc.),
# uploaded directly by the client via presigned PUT URLs generated by the
# identity_service Lambda's PRESIGN_DOCUMENT_UPLOAD action.
# ---------------------------------------------------------------------------

resource "aws_s3_bucket" "driver_documents" {
  bucket = "${var.project_name}-driver-documents-${var.environment}"
}
...
resource "aws_lambda_function" "identity_service" {
  ...
  environment {
    variables = {
      KWELLA_TABLE_NAME       = aws_dynamodb_table.kwella_core.name
      DRIVER_DOCUMENTS_BUCKET = aws_s3_bucket.driver_documents.id
    }
  }
}
...
resource "aws_apigatewayv2_route" "identity_documents_presign" {
  api_id             = aws_apigatewayv2_api.kwella_http_api.id
  route_key          = "POST /identity/documents/presign"
  target             = "integrations/${aws_apigatewayv2_integration.identity_service.id}"
  authorization_type = "CUSTOM"
  authorizer_id      = aws_apigatewayv2_authorizer.kwella_lambda_authorizer.id
}
