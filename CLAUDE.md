...
* **Plugin/Package First Approach:** Before implementing a feature from scratch, check whether a mature package/plugin already exists on pub.dev. Evaluate candidates on maintenance activity, community adoption, documentation quality, compatibility with our Flutter version, and security considerations, and prefer a proven package over a custom implementation for common functionality (e.g. OTP inputs, phone number formatting, maps/location, animations, UI components, auth helpers, common utilities). Only build custom when no suitable package exists, existing packages don't meet requirements, or the behavior is a core product differentiator.
