...
* **Anti-Over-Engineering Mandate:** Prioritize production-grade open-source packages and native SDKs[cite: 1, 3]. Do not build custom internal utilities if a stable out-of-the-box solution exists[cite: 1, 3].
...
* **Plugin/Package First Approach:** Before implementing a feature from scratch, check whether a mature package/plugin already exists on pub.dev[cite: 1]. Evaluate candidates on maintenance activity, community adoption, documentation quality, Flutter version compatibility, and security[cite: 1]. Prefer a proven package over a custom implementation for common functionality (e.g., OTP inputs, phone number formatting, maps/location, animations, UI components, auth helpers, common utilities)[cite: 1]. Only build custom when no suitable package exists, existing packages do not meet requirements, or the behavior is a core product differentiator[cite: 1].
