...
class ActiveRideOffer {
  ...
  factory ActiveRideOffer.fromPushNotification(Map<String, dynamic> json) {
    ...
    if (baseFareValue == null) {
      throw FormatException('Missing baseFare/base_fare in push notification payload.');
    }

    final baseFare = _parseDouble(baseFareValue);
    final pickupLocation = json['pickupLocation'] as String?
            ?? json['pickup_location'] as String?
            ?? _coordinateLabel(json, prefix: 'pickup')
            ?? 'Pickup location unavailable';
    final dropoffLocation = json['dropoffLocation'] as String?
  ...
  static String? _coordinateLabel(Map<String, dynamic> json, {required String prefix}) {
    final latitude = _parseDouble(json['${prefix}Latitude'] ?? json['${prefix}_latitude'] ?? json['${prefix}Lat'] ?? json['${prefix}_lat']);
    final longitude = _parseDouble(json['${prefix}Longitude'] ?? json['${prefix}_longitude'] ?? json['${prefix}Lon'] ?? json['${prefix}_lon'] ?? json['${prefix}Lng'] ?? json['${prefix}_lng']);
  ...
  static double _parseDouble(Object? value) {
    ...
    throw FormatException('Expected numeric value for base fare or coordinate, got: $value');
  }
